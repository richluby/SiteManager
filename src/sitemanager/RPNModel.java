...
public class RPNModel {
	 ...
	 *             the stack
	 * @throws IOException thrown when the swapping operation has completed
	 *             successfully to provide notification, or when the stack has
	 *             been cleared */
	public double parseAction(char operator) throws NoSuchElementException, IOException{
		double result = 0;
		if (operator == COMMANDS.CLEAR_STACK) {
			stack.clear();
			throw new IOException("Clearing stack...");
		}
		if (!stack.isEmpty()) {
			...
			switch (operator) {
				...
				case 'M' ://mod the top two items
					...
					stack.push(result);
					break;
				case COMMANDS.SWAP_TOP_STACK ://swap top two stacks
					result = b;
					stack.push(a);
					stack.push(result);
					throw new IOException("Swapping");
					//break;
			}
		} else {
